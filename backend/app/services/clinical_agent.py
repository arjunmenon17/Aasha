...
from app.models.models import (
    Patient, SymptomLog, ClinicalAssessment, EscalationEvent
)
...
SYMPTOM_NL_MAP = {
    ...
    "vision": {
        ...
        "3": "seeing spots flashing lights photopsia visual disturbance severe",
    },
    "swelling": {
        "1": "",
...
async def assemble_patient_context(
    ...
    esc_result = await db.execute(
        ...
        .where(EscalationEvent.patient_id == patient.id)
...
def build_retrieval_query(responses: dict, patient: Patient) -> str:
    ...
    query = " ".join(terms) if terms else "routine antenatal check normal pregnancy"
    return query
        ...
        try:
            response = client.messages.create(
                ...
                messages=[
                    ...
                    {"role": "assistant", "content": '{"risk_tier":'},
                ],
            )
...
async def persist_assessment(
    ...
    assessment = ClinicalAssessment(
        ...
        moorcheh_chunk_ids=[c["chunk_id"] for c in protocol_chunks] if protocol_chunks else [],
    )
    db.add(assessment)
    await db.flush()
    ...
    if risk_tier >= 2:
        ...
        concern = assessment_data.get("primary_concern", "Clinical concern detected")
    ...
    logger.info(
        f"Assessment complete for {patient.name}: Tier {risk_tier} — "
...
async def persist_fallback_assessment(
    ...
    assessment = ClinicalAssessment(
        ...
        symptom_log_id=symptom_log.id,
        risk_tier=2,
        primary_concern="Clinical assessment system error — conservative escalation applied",
        clinical_reasoning=f"Automated Tier 2 fallback due to system error: {error_msg}",
        full_assessment={"fallback": True, "error": error_msg},
        recommended_actions=["CHW should contact patient for manual assessment"],
        uncertainty_flags=["Automated assessment failed — manual review required"],
    )
    db.add(assessment)
